backward <- function (p) {
	...
	repeat {
		...
		results <- p$parply(unique(p$tab$block),function (b) {
			...
			f.alt <- buildmer:::build.formula(p$dep,p$tab[-i,],emptyenv())
			m.alt <- p$fit(p,f.alt)
		...
		p$tab <- p$tab[-remove,]
		p$formula <- build.formula(p$dep,p$tab,emptyenv())
...
order <- function (p) {
	reorder <- function (p,tab) {
		...
		repeat {
			have <- p$tab
			cur <- p$fit(p,build.formula(p$dep,have,emptyenv()))
			conv <- converged(cur)
			if (conv) break
			p <- reduce.model(p,conv)
			if (!any(!is.na(p$tab$block))) return(p)
		...
		repeat {
			...
			mods <- p$parply(unique(check$block),function (b,check,have,p) {
				...
				form <- buildmer:::build.formula(p$dep,tab,emptyenv())
			...
			if (length(unique(check[winner,'block'] == 1))) cur <- mods[[winner[1]]] else {
				...
				form <- build.formula(p$dep,have,emptyenv())
				cur <- p$fit(p,form)
				conv <- conv(cur)
				if (!conv) {
			...
			progress(p,'Updating formula: ',build.formula(p$dep,have,emptyenv()))
		}
	}
